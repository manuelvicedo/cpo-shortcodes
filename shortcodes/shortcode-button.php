...
if(!function_exists('ctsc_shortcode_button')){
	function ctsc_shortcode_button($atts, $content = null){
		...
		if($element_description != '') $output .= '<span class="ctsc-button-description">'.$element_description.'</span>';
		$output .= '</span>';
		
		$output .= '</a>';		
		return $output;
	}
	add_shortcode(ctsc_shortcode_prefix().'button', 'ctsc_shortcode_button');
}